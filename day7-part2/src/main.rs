...
fn main() {
    ...
    let occupied_space = root_directory.borrow().size();

    let smallest_big_enough_directory = root_directory
        .borrow()
        .directories()
        .into_iter()
        .map(|directory| directory.borrow().size())
        .filter(|size| total_disk_space - occupied_space + size >= 30000000)
        .min()
        .unwrap();

    println!("{smallest_big_enough_directory}");
}
